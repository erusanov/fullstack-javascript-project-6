...
export const buildApp = async (options = {}) => {
  ...
  const effectiveKnexConfig = options.knexConfig
    ?? knexConfig[process.env.NODE_ENV || 'development'];

  if (process.env.NODE_ENV !== 'test') {
    const knexInstance = knex(effectiveKnexConfig);
  ...
  app.decorateReply('t', (key, i18nextOptions) => i18next.t(key, i18nextOptions));

  fastifyPassport.registerUserDeserializer(
    async (user) => {
      const existingUser = await app.objection.models.user
        .query()
        .findById(user.id);
  ...
  await app.register(fastifyObjectionjs, {
    knexConfig: effectiveKnexConfig,
  ...
  app.get(ROUTES.ROOT.URL, { name: ROUTES.ROOT.NAME }, (request, reply) => reply.view('index.pug'));
